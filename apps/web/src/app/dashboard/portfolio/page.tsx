  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: t("properties"), value: totals.properties, icon: Building2, color: "bg-blue-50 text-blue-600" },
          { label: t("totalRooms"), value: totals.rooms, icon: BedDouble, color: "bg-purple-50 text-purple-600" },
          { label: t("avgOccupancy"), value: `${totals.avgOccupancy}%`, icon: BarChart3, color: "bg-green-50 text-green-600" },
          { label: t("totalRevenue"), value: `$${totals.totalRevenue.toLocaleString()}`, icon: DollarSign, color: "bg-amber-50 text-amber-600" },
        ].map(kpi => (
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        <div className="p-4 border-b border-slate-100">
          <h3 className="font-semibold text-slate-900">{t("propertyPerformance")}</h3>
        </div>
        {mapped.length === 0 ? (
          <div className="py-12 text-center">
            <Building2 size={40} className="mx-auto text-slate-300 mb-3" />
            <h3 className="text-lg font-semibold text-slate-900 mb-1">{t("noProperties")}</h3>
            <p className="text-sm text-slate-500">{t("noPropertiesDesc")}</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left text-sm text-slate-500 border-b border-slate-100">
                  <th className="px-4 py-3 font-medium">{t("property")}</th>
                  <th className="px-4 py-3 font-medium text-center">{tc("room")}</th>
                  <th className="px-4 py-3 font-medium text-center">{t("occupancy")}</th>
                  <th className="px-4 py-3 font-medium text-right">ADR</th>
                  <th className="px-4 py-3 font-medium text-right">RevPAR</th>
                  <th className="px-4 py-3 font-medium text-right">{t("totalRevenue")}</th>
              ...
              <tbody>
                {mapped.map(p => (
                  <tr
                    ...
                    onClick={async () => {
                      try {
                        await api.auth.switchProperty(p.id);
                        window.location.href = "/dashboard";
                      } catch (e: any) {
                        alert(e.message || "Failed to switch property");
                      }
                    }}
                  >
